...
from manexp_web_lists.core.logging_config import log_sub_section
...
def fetch_taxa() -> None:
    ...
    # 4. Clean taxa
    log_sub_section("Cleaning taxa")
    cleaned_taxa = taxo_cleaner(raw_taxa)

    # 5. Add translations
    log_sub_section("Translating taxa")
    translated_taxa = taxo_translator(cleaned_taxa)

    # 6. Add icon for each taxon
    log_sub_section("Adding icons to taxa")
    iconed_taxa = icon_generator(translated_taxa)

    # 7. Add color for each taxon
    log_sub_section("Adding colors to taxa")
